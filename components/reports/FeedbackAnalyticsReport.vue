...
const viewFeedbackDetails = (feedback: any) => {
  ...
  const ratingStars = '⭐'.repeat(feedback.rating || 0)
  alert(`Feedback Details:\n\nGuest: ${feedback.guest?.full_name || 'Unknown'}\nRating: ${ratingStars} (${feedback.rating}/5)\nRoom: ${feedback.stay?.room?.room_number || 'N/A'} (${feedback.stay?.room?.category || 'N/A'})\nStay: ${formatDate(feedback.stay?.check_in_date)} - ${formatDate(feedback.stay?.check_out_date)}\nAmount: ${feedback.stay?.total_amount ? formatCurrency(feedback.stay.total_amount) : 'N/A'}\nNationality: ${feedback.guest?.nationality || 'Unknown'}\n\nComment: "${feedback.note || 'No comment provided'}"\n\nDate: ${formatDate(feedback.created_at)}`)
}
