<template>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label
                    for="room_category_filter"
                    class="block text-sm font-medium text-gray-700 mb-1"
                    >Room Category Filter</label
                >
                <Dropdown
                    id="room_category_filter"
            ...
            <div>
                ...
                <MultiSelect
                    id="manual_room"
                    :modelValue="stayForm.rooms"
                    @update:modelValue="updateStay('rooms', $event)"
            ...
            <div>
                <label
                    for="manual_check_out"
                    class="block text-sm font-medium text-gray-700 mb-1"
                    >Check-out Date</label
                >
                <Calendar
                    id="manual_check_out"
                    :modelValue="stayForm.check_out_date"
            ...
            <div v-if="selectedRoomsDisplay.length > 0" class="col-span-2">
                <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    <div
                        v-if="
                            roomsByCategory &&
                            Object.keys(roomsByCategory).length > 1
                        "
                        class="mb-3"
                    >
                        <div
                            v-for="(rooms, category) in roomsByCategory"
                            :key="category"
        ...
        <h4 class="font-semibold text-gray-700 border-b pb-2 pt-4">
            Identity Document (Required)
        </h4>
        <Dropdown
            :modelValue="docForm.document_type"
            @update:modelValue="updateDoc('document_type', $event)"
        ...
        >
            <template #empty
                ><p>Drag and drop front side file here to upload.</p></template
            >
        </FileUpload>
        <div class="flex items-center mt-2">
            <Checkbox inputId="backside-check" v-model="showBackside" :binary="true" />
            <label for="backside-check" class="ml-2 text-sm text-gray-600">Add back side of document</label>
        </div>

        <FileUpload
            v-if="showBackside"
            name="document_back"
            @select="$emit('file-select-back', $event)"
            :multiple="false"
            accept="image/*,application/pdf"
            :maxFileSize="2000000"
            ref="walkinUploaderBack"
            class="mt-4"
        >
            <template #empty
                ><p>Drag and drop back side file here to upload.</p></template
            >
        </FileUpload>
    </div>
</template>
...
const { data: roomCategoriesData, isLoading: categoriesLoading } =
    useFetchRoomCategories();
...
const availableRooms = computed(() => {
    ...
    return roomsData.value.results.map((room: any) => ({
        label:
            selectedRoomCategory.value && selectedRoomCategory.value !== null
...
const clearUploader = () => {
    ...
    if (walkinUploaderBack.value) {
        walkinUploaderBack.value.clear();
    }
